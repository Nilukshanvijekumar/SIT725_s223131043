...
const calculateAveragePublicationYear = (bookList = books) => {
  ...
  const totalYears = bookList.reduce((total, book) => {
    if (!book || typeof book.year !== 'number' || Number.isNaN(book.year)) {
      throw new Error('Each book must contain a valid numeric year');
    }

    return total + book.year;
  }, 0);

  return totalYears / bookList.length;
};

module.exports = {
  getAllBooks,
  getBookById,
  calculateAveragePublicationYear
};
